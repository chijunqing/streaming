...
public class MyHbaseState<T> implements IBackingMap<T> {
	 ...
	 private static final Map<StateType, Serializer> DEFAULT_SERIALZERS = Maps.newHashMap();  
			    ...
			    public MyHbaseState(final Options<T> options, int partitionNum) {  
			    ...
			    protected static class HbaseFactory<T> implements StateFactory {  
			        ...
			        public State makeState(Map conf, IMetricsContext metrics,  
			            ...
			            IBackingMap state = new MyHbaseState(options,  partitionIndex);  
			    ...
			    public void multiPut(List<List<Object>> keys, List<T> values) {  
			        ...
			        for (int i = 0; i < keys.size(); i++) {  
			            ...
			            put.addColumn(this.options.columFamily.getBytes(),  this.options.qualifier.getBytes(),  this.options.serializer.serialize(val));  
			            puts.add(put);  
			        }  
			        try {  
